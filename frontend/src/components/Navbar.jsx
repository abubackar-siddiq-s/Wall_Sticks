          ...
          {open && (
            ...
            >
              <motion.div
                ...
                className="absolute right-0 top-0 bottom-0 w-[80%] max-w-xs bg-white p-6 flex flex-col gap-6 shadow-2xl overflow-y-auto"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between pb-2 border-b border-black/5">
                  <div className="flex items-center gap-2">
                    <img src="/logo.jpeg" alt="WallSticks Logo" className="w-8 h-8 object-contain rounded-lg" onError={(e) => { e.currentTarget.style.display = 'none' }} />
                    <span className="font-extrabold text-base tracking-tight">Wall<span className="text-brand-yellow">Sticks</span></span>
                  </div>
                  <button onClick={() => setOpen(false)} aria-label="Close menu" className="p-2 rounded-full hover:bg-brand-smoke transition-colors"><X size={22} /></button>
                </div>

                <nav className="flex flex-col gap-4">
                  {links.map((l) => (
                    <NavLink key={l.to} to={l.to} onClick={() => setOpen(false)} className={({ isActive }) => `text-lg font-semibold transition-colors ${isActive ? 'text-brand-black font-bold' : 'text-black/70 hover:text-black'}`}>
                      {l.label}
                    </NavLink>
                  ))}
                </nav>
                ...
                <div className="flex flex-col gap-4 pt-4 border-t border-black/10">
                  {isCustomerLoggedIn ? (
                    <>
                      <NavLink to="/wishlist" onClick={() => setOpen(false)} className="text-base font-semibold flex items-center justify-between">
                        <span>Wishlist</span>
                        {wishItems.length > 0 && <span className="bg-brand-yellow text-xs font-bold px-2 py-0.5 rounded-full">{wishItems.length}</span>}
                      </NavLink>
                      <NavLink to="/my-orders" onClick={() => setOpen(false)} className="text-base font-semibold">My Orders</NavLink>
                      <NavLink to="/cart" onClick={() => setOpen(false)} className="text-base font-semibold flex items-center justify-between">
                        <span>Cart</span>
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 text-red-600 font-semibold text-sm pt-2"
                      >
